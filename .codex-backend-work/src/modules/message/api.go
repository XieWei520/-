...
type Message struct {
	...
	backupStore         messageBackupStore
...
func New(ctx *config.Context) *Message {
	...
	m := &Message{
		...
		backupStore:         newMessageBackupStore(ctx),
...
func (m *Message) Route(r *wkhttp.WKHttp) {
	message := r.Group("/v1/message", m.ctx.AuthMiddleware(r))
	{
		...
		message.POST("/backup", m.backup)                         // 上传消息备份
		message.GET("/recovery", m.recovery)                      // 下载最新消息备份
...
func (m *Message) sendMsg(c *wkhttp.Context) {
	...
	if req.ReceiveChannelType == common.ChannelTypeGroup.Uint8() {
		groupInfo, err := m.groupService.GetGroupWithGroupNo(req.ReceiveChannelID)
		if err != nil {
			m.Error("查询群资料错误", zap.Error(err))
			c.ResponseError(errors.New("查询群资料错误"))
			return
		}
		member, err := m.groupService.GetMember(req.ReceiveChannelID, uid)
		if err != nil {
			m.Error("查询发送者群成员身份错误", zap.Error(err))
			c.ResponseError(errors.New("查询发送者群成员身份错误"))
			return
		}
		if member == nil {
		...
		if member.ForbiddenExpirTime > time.Now().Unix() {
			c.ResponseError(errors.New("当前账号已被禁言，无法发送消息"))
			return
		}
		if groupInfo.Forbidden == 1 && !isGroupManagerOrCreatorRole(member.Role) {
			c.ResponseError(errors.New("当前群已全员禁言，仅群主、管理员和机器人可以发言"))
			return
		}
	}
...
func isGroupManagerOrCreatorRole(role int) bool {
	return role == group.MemberRoleCreator || role == group.MemberRoleManager
}
...
func (m *Message) messageReaded(c *wkhttp.Context) {
	...
	if syncMsg == nil || len(syncMsg.Messages) <= 0 {
		m.Debug("没有读取到消息，按幂等已读处理", zap.Strings("messages", req.MessageIDs))
		c.ResponseOK()
		return
	}
...
func (m *Message) setMessageExtraVersion(uid, channelID string, channelType uint8, source string, messageExtraVersion int64) error {
	cacheKey := fmt.Sprintf("messageExtraVersion:%s%s", uid, source)
	err := m.ctx.GetRedisConn().Hset(cacheKey, fmt.Sprintf("%s-%d", channelID, channelType), fmt.Sprintf("%d", messageExtraVersion))
	if err != nil {
		return err
	}
	return m.ctx.GetRedisConn().Expire(cacheKey, messageExtraVersionCacheTTL)
}
...
func (m *Message) syncMessageExtra(c *wkhttp.Context) {
	...
	if strings.TrimSpace(req.ChannelID) == "" {
		opErr = errors.New("empty_channel_id")
		c.ResponseError(errors.New("频道ID不能为空！"))
		return
	}
...
func (m *Message) syncChannelMessage(c *wkhttp.Context) {
	var opErr error
	done := servermetrics.StartOperation("message_channel_sync")
	defer func() { done(opErr) }()
...
func (m *Message) sync(c *wkhttp.Context) {
	var opErr error
	done := servermetrics.StartOperation("message_sync")
	defer func() { done(opErr) }()
	uid := c.MustGet("uid").(string)
	var req syncReq
	if err := c.BindJSON(&req); err != nil {
	...
	messageIDs := phase6MessageIDsFromSyncResponses(resps)
	...
	messageExtraMap, messageUserExtraMap := phase6BuildMessageExtraMaps(messageExtras, messageUserExtras)
	// 用户频道偏移
...
func newSyncChannelMessageResp(resp *config.SyncChannelMessageResp, loginUID string, deviceUUID string, channelID string, channelType uint8, messageExtraDB *messageExtraDB, messageUserExtraDB *messageUserExtraDB, messageReactionDB *messageReactionDB, channelOffsetDB *channelOffsetDB, deviceOffsetDB *deviceOffsetDB, channelOffsetMessageSeq uint32) *syncChannelMessageResp {
	...
	if len(resp.Messages) > 0 {
		...
		messageIDSet := make(map[string]struct{}, len(resp.Messages))
		payloadMaps := make(map[int64]map[string]interface{}, len(resp.Messages))
		for _, message := range resp.Messages {
			payloadMap := readMessagePayloadMap(message.Payload)
			if len(payloadMap) > 0 {
				payloadMaps[message.MessageID] = payloadMap
				if _, replyMsgID, ok := replyMessageIDFromPayload(payloadMap); ok {
					messageIDs = appendMessageIDOnce(messageIDs, messageIDSet, replyMsgID)
				}
			}
			messageIDs = appendMessageIDOnce(messageIDs, messageIDSet, fmt.Sprintf("%d", message.MessageID))
		}
		...
		// 修改消息扩展字段
		for _, message := range resp.Messages {
			payloadMap := payloadMaps[message.MessageID]
			if len(payloadMap) == 0 {
				continue
			}
			replyMap, replyMsgID, ok := replyMessageIDFromPayload(payloadMap)
			if !ok {
				continue
			}
			messageExtra := messageExtraMap[replyMsgID]
			if messageExtra == nil || messageExtra.ContentEdit.String == "" {
				continue
			}
			var contentEditMap map[string]interface{}
			if err := util.ReadJsonByByte([]byte(messageExtra.ContentEdit.String), &contentEditMap); err != nil {
				log.Warn("负荷数据不是json格式！", zap.Error(err), zap.String("payload", string(messageExtra.ContentEdit.String)))
				continue
			}
			if len(contentEditMap) > 0 {
...
func appendMessageIDOnce(messageIDs []string, seen map[string]struct{}, messageID string) []string {
...
func readMessagePayloadMap(payload []byte) map[string]interface{} {
	var payloadMap map[string]interface{}
...
func replyMessageIDFromPayload(payloadMap map[string]interface{}) (map[string]interface{}, string, bool) {
	replyRaw := payloadMap["reply"]
	replyMap, ok := replyRaw.(map[string]interface{})
	if !ok || len(replyMap) == 0 {
		return nil, "", false
	}
	messageID := readReplyMessageID(replyMap["message_id"])
	if messageID == "" {
		return nil, "", false
	}
	return replyMap, messageID, true
}

func readReplyMessageID(raw interface{}) string {
