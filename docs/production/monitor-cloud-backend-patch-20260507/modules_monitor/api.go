...
func (a *API) Route(r *wkhttp.WKHttp) {
	...
	auth.PUT("/routes/:route_id", a.updateRoute)
	auth.DELETE("/routes/:route_id", a.deleteRoute)
...
func (a *API) updateRoute(c *wkhttp.Context) {
	uid := c.GetLoginUID()
	...
	existing, err := a.db.queryRouteByID(routeID)
	if err != nil {
		a.Error("query monitor route failed", zap.Error(err), zap.String("uid", uid), zap.String("route_id", routeID))
		a.writeError(c, http.StatusInternalServerError, "route_query_failed", "查询监控规则失败")
		return
	}
	...
	platform := normalize(req.Platform, monitorPlatformFeishu)
	connectorType := normalize(req.ConnectorType, monitorConnectorFeishuWebGroup)
	routeType := normalize(req.RouteType, existing.RouteType)
	if platform != monitorPlatformFeishu || connectorType != monitorConnectorFeishuWebGroup ||
		(routeType != monitorRouteFeishuWebToWukongIM && routeType != monitorRouteFeishuWebToFeishuAPI) {
		a.writeError(c, http.StatusUnprocessableEntity, "unsupported_route_type", "暂不支持该监控规则类型")
		return
	...
	sourceName := stringFromMap(req.Source, "chat_name")
	destinationType := normalize(stringFromMap(req.Destination, "type"), existing.DestinationType)
	destinationID := stringFromMap(req.Destination, "destination_id")
	destinationNo := stringFromMap(req.Destination, "group_no")
	destinationName := stringFromMap(req.Destination, "group_name")
	if destinationType == monitorDestinationFeishuOpenAPI {
	...
	senderDisplayAvatar := existing.SenderDisplayAvatar
	if destinationType == monitorDestinationWukongIMGroup {
		senderDisplayName = trimRunes(normalize(stringFromMap(senderIdentity, "display_name"), "Feishu Monitor"), 64)
		senderDisplayAvatar = trimRunes(stringFromMap(senderIdentity, "display_avatar"), 512)
	} else {
	...
	updated := &routeModel{
		...
		TodayForwardedCount: existing.TodayForwardedCount,
		LastForwardedAt:     existing.LastForwardedAt,
		IncludeText:         boolToInt(boolFromMap(req.MessagePolicy, "include_text", true)),
		IncludeLinks:        boolToInt(boolFromMap(req.MessagePolicy, "include_links", true)),
		IncludeImages:       boolToInt(boolFromMap(req.MessagePolicy, "include_images", false)),
		IncludeFiles:        boolToInt(boolFromMap(req.MessagePolicy, "include_files", false)),
	...
	if err := a.db.updateRoute(updated); err != nil {
		a.Error("update monitor route failed", zap.Error(err), zap.String("uid", uid), zap.String("route_id", routeID))
		a.writeError(c, http.StatusInternalServerError, "route_update_failed", "更新监控规则失败")
		return
	}
	_ = a.insertEvent(uid, existing.AgentID, routeID, "route_updated", "已更新飞书监控规则 "+updated.SourceName+" → "+updated.DestinationName, map[string]interface{}{"route_id": routeID})
	c.Response(map[string]interface{}{"data": routeToJSON(updated)})
}

func (a *API) deleteRoute(c *wkhttp.Context) {
	uid := c.GetLoginUID()
	routeID := strings.TrimSpace(c.Param("route_id"))
	route, err := a.db.queryRouteByID(routeID)
	if err != nil {
		a.Error("query monitor route for delete failed", zap.Error(err), zap.String("uid", uid), zap.String("route_id", routeID))
		a.writeError(c, http.StatusInternalServerError, "route_query_failed", "查询监控规则失败")
		return
	}
	if route == nil || route.UID != uid {
		a.writeError(c, http.StatusNotFound, "route_not_found", "监控规则不存在")
		return
	}
	if err := a.db.deleteRoute(uid, routeID); err != nil {
		a.Error("delete monitor route failed", zap.Error(err), zap.String("uid", uid), zap.String("route_id", routeID))
		a.writeError(c, http.StatusInternalServerError, "route_delete_failed", "删除监控规则失败")
		return
	}
	_ = a.insertEvent(uid, route.AgentID, routeID, "route_deleted", "已删除飞书监控规则 "+route.SourceName+" → "+route.DestinationName, map[string]interface{}{"route_id": routeID})
	c.Response(map[string]interface{}{"data": map[string]interface{}{"route_id": routeID, "deleted": true}})
}
