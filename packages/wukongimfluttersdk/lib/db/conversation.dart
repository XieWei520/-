...
class ConversationDB {
  ...
  Future<WKConversationMsg?> queryMsgByMsgChannelId(
    ...
    final sql =
        "SELECT ${WKDBConst.tableConversation}.*,$extraCols FROM ${WKDBConst.tableConversation} LEFT JOIN ${WKDBConst.tableConversationExtra} ON ${WKDBConst.tableConversation}.channel_id=${WKDBConst.tableConversationExtra}.channel_id AND ${WKDBConst.tableConversation}.channel_type=${WKDBConst.tableConversationExtra}.channel_type WHERE ${WKDBConst.tableConversation}.channel_id=? AND ${WKDBConst.tableConversation}.channel_type=? LIMIT 1";
  ...
  List<WKConversationMsg> dedupeByChannel(List<WKConversationMsg> list) {
    final deduped = <String, WKConversationMsg>{};
    for (final msg in list) {
      if (msg.channelID == '') {
        continue;
      }
      final key = '${msg.channelID}:${msg.channelType}';
      final current = deduped[key];
      if (current == null || _isNewerConversation(msg, current)) {
        deduped[key] = msg;
      }
    }
  ...
  Future<void> insertMsgList(List<WKConversationMsg> list) async {
    if (WKDBHelper.shared.getDB() == null) {
      return;
    }
    final dedupedConversations = dedupeByChannel(list);
    if (dedupedConversations.isEmpty) {
      return;
    }
    ...
    for (final msg in dedupedConversations) {
      final existing =
          await queryMsgByMsgChannelId(msg.channelID, msg.channelType);
      if (existing == null || existing.channelID.isEmpty) {
        insertList.add(getMap(msg, true));
      } else {
      ...
      await WKDBHelper.shared.getDB()!.transaction((txn) async {
        ...
        if (updateList.isNotEmpty) {
          for (Map<String, dynamic> value in updateList) {
            await txn.update(WKDBConst.tableConversation, value,
                where: "channel_id=? and channel_type=?",
  ...
  bool _isNewerConversation(WKConversationMsg next, WKConversationMsg current) {
    if (next.lastMsgSeq != current.lastMsgSeq) {
      return next.lastMsgSeq > current.lastMsgSeq;
    }
    return next.lastMsgTimestamp >= current.lastMsgTimestamp;
  }
  ...
  Future<int> insertOrUpdateExtra(WKConversationMsgExtra extra) async {
    if (WKDBHelper.shared.getDB() == null) {
      return 0;
    }
    return WKDBHelper.shared.getDB()!.insert(
  ...
  Future<void> insertOrUpdateExtras(List<WKConversationMsgExtra> list) async {
    if (WKDBHelper.shared.getDB() == null || list.isEmpty) {
      return;
    }
    final insertList = list.map(getExtraMap).toList(growable: false);
    await WKDBHelper.shared.getDB()!.transaction((txn) async {
  ...
  Map<String, dynamic> getExtraMap(WKConversationMsgExtra extra) {
    Map<String, dynamic> data = HashMap<String, Object>();
    data['channel_id'] = extra.channelID;
    data['channel_type'] = extra.channelType;
    data['browse_to'] = extra.browseTo;
    data['keep_message_seq'] = extra.keepMessageSeq;
    data['keep_offset_y'] = extra.keepOffsetY;
    data['draft'] = extra.draft;
    data['version'] = extra.version;
    data['draft_updated_at'] = extra.draftUpdatedAt;
    return data;
  }
}
