...
import 'package:wukongimfluttersdk/common/reconnect_policy.dart';
import 'package:wukongimfluttersdk/db/message.dart' as msg_db;
...
class _WKSocket {
  WKConnectionTransport? _transport;
  bool _isListening = false;
  static _WKSocket? _instance;
  _WKSocket._internal(this._transport);

  factory _WKSocket.newTransport(WKConnectionTransport transport) {
    if (_instance != null) {
      _instance!._destroySocket();
    }
    _instance = _WKSocket._internal(transport);
    return _instance!;
  }

  /// P0-T04: Dispose the singleton completely (for logout).
...
class WKConnectionManager {
  ...
  // P0-T01: Replace fixed delay with exponential backoff + jitter
  ...
  disconnect(bool isLogout) {
    ...
    if (isLogout) {
      ...
      WKIM.shared.options.token = '';
      if (WKIM.shared.isApp()) {
        WKIM.shared.messageManager.updateSendingMsgFail();
        WKDBHelper.shared.close();
      }
      // P0-T04: Fully dispose socket singleton on logout
      _WKSocket.disposeInstance();
      _socket = null;
    }
    _closeAll();
  ...
  _socketConnect(String addr) async {
    Logs.info("连接地址--->$addr");
    if (addr == '') {
      _connectFail('连接地址为空');
      return;
    ...
    try {
      target = WKConnectTarget.parse(addr);
    } catch (e) {
      _connectFail(e);
      return;
    }
    ...
    try {
      ...
      final transport = await WKConnectionTransportFactory.connect(
        target,
        timeout: const Duration(seconds: 5),
      );
      _socket = _WKSocket.newTransport(transport);
      _connectSuccess();
    } catch (e) {
      _connectFail(e);
    }
  ...
  /// P0-T01: Schedule reconnect with exponential backoff + jitter.
  void _scheduleReconnect() {
    if (isDisconnection) return;

    final delay = _reconnectPolicy.nextDelay();
    if (delay == -1) {
      Logs.error('重连次数已达上限(${_reconnectPolicy.maxRetries})，停止重连');
      setConnectionStatus(WKConnectStatus.fail);
      return;
    }
    Logs.debug('将在 ${delay}ms 后重连 (第${_reconnectPolicy.retryCount}次)');
  ...
  // P0-T03: Replace manual array copy with BytesBuilder
  final BytesBuilder _packetBuffer = BytesBuilder(copy: false);

  _cutDatas(Uint8List data) {
    _packetBuffer.add(data);
    Uint8List lastMsgBytes = _packetBuffer.toBytes();
    int readLength = 0;
    while (lastMsgBytes.isNotEmpty && readLength != lastMsgBytes.length) {
      ...
      if (PacketType.values[(b >> 4)] == PacketType.pong) {
        ...
        _packetBuffer.clear();
        _packetBuffer.add(bytes);
        lastMsgBytes = bytes;
      } else {
        if (packetType < 10) {
          if (lastMsgBytes.length < 5) {
            _packetBuffer.clear();
            _packetBuffer.add(lastMsgBytes);
            break;
          }
  ...
  _decodePacket(Uint8List data) {
    ...
    if (packet.header.packetType == PacketType.connack) {
      ...
      if (connackPacket.reasonCode == 1) {
        Logs.debug('连接成功！');
        // P0-T01: Reset reconnect policy on successful connection
  ...
  _closeAll() {
    ...
    _stopHeartTimer();
    // P0-T03: Clear packet buffer on disconnect
    _packetBuffer.clear();
    if (_socket != null) {
      _socket!.close();
    }
      ...
      connectivityResult.then((value) {
        if (value.contains(ConnectivityResult.none)) {
          ...
          if (lastConnectivityResult != null &&
            ...
            // P0-T01: Network switch detected, reset backoff for immediate reconnect
            isReconnection = true;
            _reconnectPolicy.reset();
          }
          if (isReconnection) {
  ...
  _resendMsg() async {
    _removeSendingMsg();
    // Resend in-memory queued messages
    ...
    // P0-T02: Recover sending messages persisted in DB (crash recovery)
    try {
      final pendingMsgs = await msg_db.MessageDB.shared
          .recoverSendingMessages(recentSeconds: 300);
      for (final msg in pendingMsgs) {
        if (!_sendingMsgMap.containsKey(msg.clientSeq)) {
          Logs.debug("从DB恢复发送消息：${msg.clientSeq}");
          WKIM.shared.connectionManager.sendMessage(msg);
        }
      }
    } catch (e) {
      Logs.error('恢复发送中消息失败: $e');
    }
  }
