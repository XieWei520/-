...
class WKConversationManager {
  ...
  Future<int> getMsgExtraMaxVersion() async {
    return getExtraMaxVersion();
  }
  ...
  Future<void> updateMsgExtra(WKConversationMsgExtra extra) async {
    if (extra.channelID.isEmpty) {
      return;
    }
    await ConversationDB.shared.insertOrUpdateExtra(extra);
    await _refreshChannelMsg(extra.channelID, extra.channelType);
  }

  Future<void> saveSyncMsgExtras(List<WKSyncConvMsgExtra> list) async {
    if (list.isEmpty) {
      return;
    }
    final extras = <WKConversationMsgExtra>[];
    for (final item in list) {
      ...
      final extra = WKConversationMsgExtra()
        ...
        ..browseTo = item.browseTo
        ..keepMessageSeq = item.keepMessageSeq
        ..keepOffsetY = item.keepOffsetY
        ..draft = item.draft
    ...
    await ConversationDB.shared.insertOrUpdateExtras(extras);

    final uiMsgs = <WKUIConversationMsg>[];
    for (final extra in extras) {
      final uiMsg = await getWithChannel(extra.channelID, extra.channelType);
      if (uiMsg != null) {
        uiMsgs.add(uiMsg);
      }
    }
