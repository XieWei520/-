...
class WKMessageManager {
  ...
  Timer? _expireMsgCheckTimer;
  bool _expireMsgSweepRunning = false;
  ...
  WKMessageContent? getMessageModel(int type, dynamic json) {
    ...
    content.robotID = WKDBConst.readString(json, 'robot_id');
    // 回复
  ...
  Future<List<WKMsg>> getWithFlame() {
    return MessageDB.shared.queryWithFlame();
  }
    ...
    for (var msg in msgList) {
      for (var extra in list) {
        ...
        if (msg.messageID == extra.messageID) {
          ...
          msg.wkMsgExtra!.isPinned = extra.isPinned;
  ...
  WKMsgExtra wkSyncExtraMsg2WKMsgExtra(
    ...
    extra.isPinned = extraMsg.isPinned;
...
//                oldestOrderSeq = 0;
  ...
  void startExpireMessageCheckTimer() {
    stopExpireMessageCheckTimer();
    final interval = WKIM.shared.options.expireMsgCheckInterval;
    if (interval <= Duration.zero) {
      return;
    }
    _expireMsgCheckTimer = Timer.periodic(interval, (_) {
      unawaited(
        runExpireMessageCheck(limit: WKIM.shared.options.expireMsgLimit),
      );
    });
  }

  void stopExpireMessageCheckTimer() {
    if (_expireMsgCheckTimer != null) {
      _expireMsgCheckTimer!.cancel();
      _expireMsgCheckTimer = null;
    }
  }

  Future<int> runExpireMessageCheck({
    int? nowTimestamp,
    int? limit,
  }) async {
    if (_expireMsgSweepRunning) {
      return 0;
    }
    _expireMsgSweepRunning = true;
    try {
      final expiredMessages = await MessageDB.shared.queryExpiredMessages(
        nowTimestamp: nowTimestamp ??
            (DateTime.now().millisecondsSinceEpoch / 1000).truncate(),
        limit: limit ?? WKIM.shared.options.expireMsgLimit,
      );
      var deletedCount = 0;
      for (final message in expiredMessages) {
        final changed = await _markDeletedMessage(message);
        if (changed > 0) {
          deletedCount++;
        }
      }
  ...
  Future<WKMsg> _sendWithOption(WKMsg wkMsg, WKMessageContent messageContent,
    ...
    wkMsg.content = buildSendPayload(wkMsg);
  ...
  String buildSendPayload(WKMsg wkMsg) {
    ...
    final robotID = wkMsg.messageContent!.robotID.trim();
    if (robotID.isNotEmpty) {
      json['robot_id'] = robotID;
    }
  ...
  deleteWithClientMsgNo(String clientMsgNo) async {
    ...
    await _markDeletedMessage(wkMsg);
  }

  Future<int> _markDeletedMessage(WKMsg wkMsg) async {
    var map = <String, Object>{};
    map['is_deleted'] = 1;
    int result;
    if (wkMsg.clientSeq != 0) {
      result = await MessageDB.shared.updateMsgWithField(map, wkMsg.clientSeq);
    } else if (wkMsg.clientMsgNO != '') {
      result = await MessageDB.shared.updateMsgWithFieldAndClientMsgNo(
        map,
        wkMsg.clientMsgNO,
      );
    } else if (wkMsg.messageID != '') {
      result = await MessageDB.shared.deleteWithMessageIDs(<String>[
        wkMsg.messageID,
      ]);
    } else {
      return 0;
    ...
    if (result > 0) {
      wkMsg.isDeleted = 1;
      if (wkMsg.clientMsgNO != '') {
        _setDeleteMsg(wkMsg.clientMsgNO);
      }
      setRefreshMsg(wkMsg);
      var coverMsg = await ConversationDB.shared.queryMsgByMsgChannelId(
        wkMsg.channelID,
        wkMsg.channelType,
      );
      if (coverMsg != null && coverMsg.lastClientMsgNO == wkMsg.clientMsgNO) {
        var tempMsg = await MessageDB.shared.queryMaxOrderSeqMsgWithChannel(
          wkMsg.channelID,
          wkMsg.channelType,
        );
        if (tempMsg != null) {
          var uiMsg = await WKIM.shared.conversationManager.saveWithLiMMsg(
            tempMsg,
            0,
          );
          if (uiMsg != null) {
