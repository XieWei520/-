...
class DownloadManager {
  ...
  DownloadTask? getTaskByUrl(String url) {
    return _tasks.values.firstWhere((t) => t.url == url, orElse: () => throw Exception('Not found'));
  }
  ...
  Future<DownloadResult> download({
    ...
    // Determine save path
    final directory = savePath ?? (await _getDownloadDirectory());
  ...
  Future<String?> downloadImage(String url, {String? savePath}) async {
    ...
    final result = await download(url: url, fileName: fileName, savePath: savePath);
  ...
  void _notifyUpdate(DownloadTask task) {
    _downloadController.add(DownloadUpdate(
      taskId: task.id,
      task: task,
    ));
  }
