...
constexpr auto kNetworkCaptureChannel = "wukong/feishu_network_capture";
...
class WebviewWindowsPlugin : public flutter::Plugin {
 ...
 private:
  ...
  std::unique_ptr<flutter::MethodChannel<flutter::EncodableValue>>
      network_capture_channel_;

  WNDCLASS window_class_ = {};
  flutter::TextureRegistrar* textures_;
  flutter::BinaryMessenger* messenger_;
  bool network_capture_running_ = false;

  bool InitPlatform();
  void AttachNetworkCapture(WebviewBridge* bridge);
  void StopAllNetworkCapture();
  void EmitNetworkCaptureUnavailable(const std::string& message);
  void DisableNetworkCaptureAsUnavailable(const std::string& message);
  void EmitNetworkCaptureEvent(const WebviewNetworkEvent& event);
  void HandleNetworkCaptureMethodCall(
...
WebviewWindowsPlugin::WebviewWindowsPlugin(flutter::TextureRegistrar* textures,
  ...
  network_capture_channel_ =
      std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
          messenger_, kNetworkCaptureChannel,
          &flutter::StandardMethodCodec::GetInstance());
  network_capture_channel_->SetMethodCallHandler(
      [this](const auto& call, auto result) {
        HandleNetworkCaptureMethodCall(call, std::move(result));
      });
}

WebviewWindowsPlugin::~WebviewWindowsPlugin() {
  if (network_capture_channel_) {
    network_capture_channel_->SetMethodCallHandler(nullptr);
  }
  instances_.clear();
...
void WebviewWindowsPlugin::AttachNetworkCapture(WebviewBridge* bridge) {
  bridge->SetNetworkEventCallback(
      [this](const WebviewNetworkEvent& event) { EmitNetworkCaptureEvent(event); });
}

void WebviewWindowsPlugin::StopAllNetworkCapture() {
  network_capture_running_ = false;
  for (auto& item : instances_) {
    item.second->StopNetworkCapture();
  }
}

void WebviewWindowsPlugin::EmitNetworkCaptureUnavailable(
  ...
  auto args = std::make_unique<flutter::EncodableValue>(flutter::EncodableMap{
      {flutter::EncodableValue("error"),
       flutter::EncodableValue("network_capture_unavailable")},
      {flutter::EncodableValue("message"), flutter::EncodableValue(message)},
  });
  network_capture_channel_->InvokeMethod("networkUnavailable", std::move(args));
}

void WebviewWindowsPlugin::DisableNetworkCaptureAsUnavailable(
    const std::string& message) {
  StopAllNetworkCapture();
  EmitNetworkCaptureUnavailable(message);
}

void WebviewWindowsPlugin::EmitNetworkCaptureEvent(
    const WebviewNetworkEvent& event) {
  if (!network_capture_channel_ || !network_capture_running_) {
    return;
  }

  auto args = std::make_unique<flutter::EncodableValue>(flutter::EncodableMap{
      {flutter::EncodableValue("id"), flutter::EncodableValue(event.id)},
      {flutter::EncodableValue("observed_at"),
       flutter::EncodableValue(event.observed_at)},
      {flutter::EncodableValue("source"), flutter::EncodableValue(event.source)},
      {flutter::EncodableValue("url"), flutter::EncodableValue(event.url)},
      {flutter::EncodableValue("method"), flutter::EncodableValue(event.method)},
      {flutter::EncodableValue("status_code"),
       flutter::EncodableValue(event.status_code)},
      {flutter::EncodableValue("mime_type"),
       flutter::EncodableValue(event.mime_type)},
      {flutter::EncodableValue("payload_preview"),
       flutter::EncodableValue(event.payload_preview)},
      {flutter::EncodableValue("resource_type"),
       flutter::EncodableValue(event.resource_type)},
      {flutter::EncodableValue("document_url"),
       flutter::EncodableValue(event.document_url)},
      {flutter::EncodableValue("initiator_type"),
       flutter::EncodableValue(event.initiator_type)},
      {flutter::EncodableValue("initiator_url"),
       flutter::EncodableValue(event.initiator_url)},
      {flutter::EncodableValue("initiator_stack_url"),
       flutter::EncodableValue(event.initiator_stack_url)},
      {flutter::EncodableValue("initiator_line_number"),
       flutter::EncodableValue(event.initiator_line_number)},
      {flutter::EncodableValue("initiator_column_number"),
       flutter::EncodableValue(event.initiator_column_number)},
      {flutter::EncodableValue("frame_id"),
       flutter::EncodableValue(event.frame_id)},
...
void WebviewWindowsPlugin::HandleNetworkCaptureMethodCall(
    const flutter::MethodCall<flutter::EncodableValue>& method_call,
    std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
  if (method_call.method_name().compare("start") == 0) {
    network_capture_running_ = true;
    for (auto& item : instances_) {
      AttachNetworkCapture(item.second.get());
      const auto start_result = item.second->StartNetworkCapture();
      if (!start_result.success) {
        DisableNetworkCaptureAsUnavailable(start_result.error_message);
        return result->Error("network_capture_unavailable",
  ...
  if (method_call.method_name().compare("stop") == 0) {
    StopAllNetworkCapture();
    return result->Success(flutter::EncodableMap{
  ...
  webview_host_->CreateWebview(
      ...
      [shared_result, this](std::unique_ptr<Webview> webview,
        ...
        auto bridge = std::make_unique<WebviewBridge>(
            ...
            std::move(webview));
        AttachNetworkCapture(bridge.get());
        if (network_capture_running_) {
          const auto start_result = bridge->StartNetworkCapture();
          if (!start_result.success) {
            DisableNetworkCaptureAsUnavailable(start_result.error_message);
          }
        }
