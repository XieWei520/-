...
WebviewBridge::~WebviewBridge() {
  ...
  if (webview_) {
    webview_->OnNetworkEvent(nullptr);
    webview_->StopNetworkCapture();
  }
  texture_registrar_->UnregisterTexture(texture_id_);
}

WebviewNetworkCaptureStartResult WebviewBridge::StartNetworkCapture() {
  return webview_->StartNetworkCapture();
}

bool WebviewBridge::StopNetworkCapture() {
  return webview_->StopNetworkCapture();
}

void WebviewBridge::SetNetworkEventCallback(
    Webview::NetworkEventCallback network_event_callback) {
  network_event_callback_ = std::move(network_event_callback);
  webview_->OnNetworkEvent([this](const WebviewNetworkEvent& event) {
