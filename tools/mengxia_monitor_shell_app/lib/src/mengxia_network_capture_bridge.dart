...
class MengxiaNetworkCaptureBridge {
  MengxiaNetworkCaptureBridge({MethodChannel? channel})
    : channel =
  ...
  final StreamController<MengxiaNetworkCaptureEvent> _events =
      StreamController<MengxiaNetworkCaptureEvent>.broadcast();
  final StreamController<String> _unavailableErrors =
      StreamController<String>.broadcast();
  bool _disposed = false;

  Stream<MengxiaNetworkCaptureEvent> get events => _events.stream;
  ...
  Future<void> dispose() async {
    ...
    try {
      ...
      // Stop is best-effort; disposing streams is more important.
    } finally {
      await _events.close();
...
MengxiaNetworkCaptureEvent _eventFromMap(Map<Object?, Object?> map) {
  return MengxiaNetworkCaptureEvent(
    ...
    statusCode: _intValue(map['status_code']),
...
MengxiaNetworkEventSource _sourceFromString(String value) {
  for (final source in MengxiaNetworkEventSource.values) {
    if (source.name == value) {
      return source;
    }
  }
  return MengxiaNetworkEventSource.unknown;
}
