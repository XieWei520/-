...
const findTopMenuByPath = (menu: Menu.MenuOptions[], path: string): Menu.MenuOptions | undefined => {
  for (const item of menu) {
    if (item.path === path) return item;
    if (item.children?.some(child => child.path === path || findTopMenuByPath([child], path))) return item;
  }
};
...
watch(
  ...
  () => {
    ...
    const menuItem = findTopMenuByPath(menuList.value, route.path);
    splitActive.value = menuItem?.path || route.path;
