    ...
    <div class="flex-1 el-card border-none flex-col box-border overflow-hidden">
      ...
      <div class="flex-1 overflow-hidden p-12px">
        ...
        >
          ...
          <el-table-column v-for="item in displayColumns" v-bind="item" :key="item.prop">
...
const toolbarColumns = computed(() =>
  column
    .filter(item => item.prop && item.prop !== 'operation')
    .map(item => ({ key: item.prop as string, label: item.label || item.prop || '' }))
);
const visibleColumnKeys = ref<string[]>([]);
const displayColumns = computed(() =>
  column.filter(item => item.prop === 'operation' || visibleColumnKeys.value.includes(item.prop as string))
);
...
const onBatch = () => {
  ElMessageBox.confirm(`已选择 ${selectedRows.value.length} 个用户，批量操作暂未开放。`, '批量操作', {
