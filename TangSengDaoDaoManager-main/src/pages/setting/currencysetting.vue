        ...
        <div class="h-full overflow-auto">
          <div class="w-540px">
            <el-form :model="formData" label-width="160px">
              ...
              <el-divider content-position="left">启动维护模式</el-divider>
              <el-form-item label="维护模式：">
                <el-radio-group v-model="formData.maintenance_enabled">
                  <el-radio :label="1">开启</el-radio>
                  <el-radio :label="0">关闭</el-radio>
                </el-radio-group>
              </el-form-item>
              <template v-if="formData.maintenance_enabled === 1">
                <el-form-item label="维护标题：">
                  <el-input v-model="formData.maintenance_title" maxlength="80" show-word-limit />
                </el-form-item>
...
const formData = reactive({
  ...
  channel_pinned_message_max_count: 10,
...
const initData = () => {
  getAppconfigGet().then((res: any) => {
    ...
    formData.channel_pinned_message_max_count = res.channel_pinned_message_max_count ?? 10;
    formData.can_modify_api_url = res.can_modify_api_url ?? 0;
    formData.maintenance_enabled = res.maintenance_enabled ?? 0;
    formData.maintenance_title = res.maintenance_title || '';
    formData.maintenance_message = res.maintenance_message || '';
  });
};
...
const onSaveClick = () => {
  confirmHighRiskAction('保存通用设置', '确认保存通用设置和维护模式配置吗？')
    .then(({ reason }) => {
      loginButLoading.value = true;
      return updateAppconfigPost({ ...formData, reason });
    })
    .then((_res: any) => {
