...
param(
  ...
  [string]$RemoteBackupRoot = '/opt/wukongim-prod/backups/phase4-sync-load-reduction-source-sync',
  [string]$PatchPath = '',
  [string]$SshKeyPath = '',
  [switch]$Run,
...
$ExpectedManifestRows = @(
  '356977a314109ec6fe3304db89e666f5afceb12a91e106fc30cf63d33d3816c2  modules/message/api.go',
  '5626fe19f7e9d48bf234ae41a966350fb9d980632cb009bba2d30ab5ea3d55d5  modules/message/phase4_sync_load_test.go'
)
...
function Resolve-PatchFile {
  ...
  $repoRoot = Resolve-RepoRoot
  return (Resolve-Path -LiteralPath (Join-Path $repoRoot 'deploy\production\backend-optimization\patches\0002-phase4-sync-load-reduction.patch')).Path
}
...
function Invoke-RemoteBash {
  ...
  $remoteScriptFile = Join-Path ([System.IO.Path]::GetTempPath()) "phase4-remote-bash-$([guid]::NewGuid().ToString('N')).sh"
  $cmdFile = Join-Path ([System.IO.Path]::GetTempPath()) "phase4-remote-bash-$([guid]::NewGuid().ToString('N')).cmd"
  ...
  if ($process.ExitCode -ne 0) {
    throw "Remote Phase 4 sync load reduction prepare command failed with exit code $($process.ExitCode)."
  }
}
...
function Assert-ReviewedManifest {
  ...
  if ($actual.Count -ne $expected.Count) {
    throw 'LocalBackendRoot files do not match reviewed Phase 4 manifest.'
  }
...
function Invoke-GitApplyPatch {
  ...
  try {
    ...
    if ($LASTEXITCODE -eq 0) {
      ...
      Write-Host 'phase4_sync_load_reduction_local_patch=applied'
      return
    }

    & git apply --reverse --check --whitespace=nowarn $patchFile
    if ($LASTEXITCODE -eq 0) {
      throw 'Phase 4 sync load reduction patch appears to be already applied to LocalBackendRoot.'
    }
    throw 'Phase 4 sync load reduction patch cannot be applied cleanly to LocalBackendRoot.'
  } finally {
    Pop-Location
  }
}
...
function Invoke-LocalTests {
  ...
  Push-Location -LiteralPath $BackendRoot
  try {
    Write-Host "Running: go test -count=1 ./modules/message -run 'TestPhase4|TestSyncSensitiveWords|TestProhibit'"
...
if (-not $Run) {
  ...
  Write-Host 'phase4_sync_load_reduction_sync_backup_dir=<created only when -Run -AllowProductionSync is used>'
  Write-Host 'Reviewed manifest: verified'
  Write-Host 'phase4_sync_load_reduction_reviewed_manifest=verified'
  Write-Host ''
...
if (-not $BuildImage) {
  throw 'Refusing to run production Phase 4 prepare without -BuildImage.'
}

$remoteTempDir = "/tmp/phase4-sync-load-reduction-sync-$([guid]::NewGuid().ToString('N'))"
...
try {
  ...
  $manifestFile = Join-Path ([System.IO.Path]::GetTempPath()) "phase4-sync-load-reduction-manifest-$([guid]::NewGuid().ToString('N')).txt"
  try {
...
if ! diff -u <(printf '%s\n' "`$manifest_text") "`$remote_tmp/.manifest"; then
  echo 'phase4_sync_load_reduction_sync=manifest_mismatch' >&2
  exit 1
fi
...
absent_manifest="`$backup_dir/.phase4_absent_files"
: > "`$absent_manifest"
...
END {
  for (path in before) {
    if (!(path in allowed)) {
      print "phase4_sync_load_reduction_build_context_unreviewed_change=" path > "/dev/stderr"
      exit 1
    }
  }
}' "`$release_paths_file" <(comm -3 "`$remote_tmp/.build-context.before.tsv" "`$remote_tmp/.build-context.after.tsv" | sed 's/^\t//'); then
  echo 'phase4_sync_load_reduction_build_context=unreviewed_change' >&2
  exit 1
fi
echo 'phase4_sync_load_reduction_build_context=verified'

echo 'phase4_sync_load_reduction_sync=applied'
echo "phase4_sync_load_reduction_sync_backup_dir=`$backup_dir"
echo "phase4_sync_load_reduction_absent_files_manifest=`$absent_manifest"
echo 'phase4_sync_load_reduction_reviewed_manifest=verified'
...
if [ "`$build_image" = '1' ]; then
  ...
  cat > "`$build_context_root/Dockerfile.tsdd" <<'PHASE4_DOCKERFILE'
...
ENTRYPOINT ["/home/app"]
PHASE4_DOCKERFILE
  cat > "`$build_context_root/.dockerignore" <<'PHASE4_DOCKERIGNORE'
.git
**/.git
...
**/*.jks
PHASE4_DOCKERIGNORE
  cat > "`$remote_tmp/docker-compose.phase4-build.yaml" <<'PHASE4_COMPOSE'
services:
  tsdd-api:
    build:
...
PHASE4_COMPOSE
  sed -i "s|__BUILD_CONTEXT_ROOT__|`$build_context_root|g" "`$remote_tmp/docker-compose.phase4-build.yaml"
  echo "phase4_sync_load_reduction_build_context_root=`$build_context_root"
  cd "`$remote_production"
  image_timestamp="`$(date -u +%Y%m%dT%H%M%SZ)"
  previous_image_tag="wukongim/tsdd-api:phase4-pre-`$image_timestamp"
  if docker image inspect wukongim/tsdd-api:production-local >/dev/null 2>&1; then
    ...
    echo "phase4_sync_load_reduction_previous_image_tag=`$previous_image_tag"
  else
    echo 'previous production image is missing: wukongim/tsdd-api:production-local' >&2
    exit 1
  fi
  docker compose --env-file .env -f "`$remote_tmp/docker-compose.phase4-build.yaml" build tsdd-api
  echo 'phase4_sync_load_reduction_build=completed'
  docker compose --env-file .env ps tsdd-api
else
  echo 'phase4_sync_load_reduction_build=skipped'
fi
"@
