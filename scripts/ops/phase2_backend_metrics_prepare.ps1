...
function Resolve-PatchFile {
  ...
  $repoRoot = Resolve-RepoRoot
  return (Resolve-Path -LiteralPath (Join-Path $repoRoot 'deploy\production\backend-metrics\patches\0001-backend-http-operation-metrics.patch')).Path
}
...
function Invoke-RemoteBash {
  ...
  if ($process.ExitCode -ne 0) {
    throw "Remote Phase 2 backend metrics prepare command failed with exit code $($process.ExitCode)."
  }
}
...
function Invoke-GitApplyPatch {
  ...
  try {
    ...
    if ($LASTEXITCODE -eq 0) {
      ...
      Write-Host 'phase2_backend_metrics_local_patch=applied'
      return
    }

    & git apply --reverse --check --whitespace=nowarn $patchFile
    if ($LASTEXITCODE -eq 0) {
      throw 'Phase 2 backend metrics patch appears to be already applied to LocalBackendRoot.'
    }
    throw 'Phase 2 backend metrics patch cannot be applied cleanly to LocalBackendRoot.'
  } finally {
    Pop-Location
  }
}
...
function Invoke-LocalTests {
  ...
  try {
    Write-Host 'Running: go test -count=1 ./pkg/metrics'
  ...
  Push-Location -LiteralPath $BackendRoot
  try {
    Write-Host "Running backend module compile gate: go test -count=1 -mod=readonly -run '^$' ./modules/message ./modules/file"
...
if (-not $Run) {
  Write-Host 'Dry run only. Add -Run -AllowProductionSync to sync Phase 2 backend metrics source files.'
...
$remoteTempDir = "/tmp/phase2-backend-metrics-sync-$([guid]::NewGuid().ToString('N'))"
...
try {
  ...
  $manifestFile = Join-Path ([System.IO.Path]::GetTempPath()) "phase2-backend-metrics-manifest-$([guid]::NewGuid().ToString('N')).txt"
  try {
...
if ! diff -u <(printf '%s\n' "`$manifest_text") "`$remote_tmp/.manifest"; then
  echo 'phase2_backend_metrics_sync=manifest_mismatch' >&2
  exit 1
fi

timestamp="`$(date +%Y%m%dT%H%M%S%z)"
backup_dir="`$remote_production/backups/phase2-backend-metrics-source-sync/`$timestamp"
mkdir -p "`$backup_dir"
...
echo 'phase2_backend_metrics_sync=applied'
echo "phase2_backend_metrics_sync_backup_dir=`$backup_dir"

if [ "`$build_image" = '1' ]; then
  cd "`$remote_production"
  docker compose --env-file .env build tsdd-api
  echo 'phase2_backend_metrics_build=completed'
  docker compose --env-file .env ps tsdd-api
else
  echo 'phase2_backend_metrics_build=skipped'
fi
"@
