...
param(
  ...
  [string]$RemoteBackupRoot = '/opt/wukongim-prod/backups/phase5-sync-backend-fastpaths-source-sync',
  [string]$PatchPath = '',
  [string]$SshKeyPath = '',
  [switch]$Run,
...
function Invoke-RemoteBash {
  ...
  $remoteScriptFile = Join-Path ([System.IO.Path]::GetTempPath()) "phase5-remote-bash-$([guid]::NewGuid().ToString('N')).sh"
  $cmdFile = Join-Path ([System.IO.Path]::GetTempPath()) "phase5-remote-bash-$([guid]::NewGuid().ToString('N')).cmd"
  ...
  if ($process.ExitCode -ne 0) {
    throw "Remote Phase 5 sync backend fastpaths prepare command failed with exit code $($process.ExitCode)."
  }
}
...
function Assert-ReviewedManifest {
  ...
  if ($actual.Count -ne $expected.Count) {
    throw 'LocalBackendRoot files do not match reviewed Phase 5 manifest.'
  }
...
function Invoke-GitApplyPatch {
  ...
  try {
    ...
    if ($LASTEXITCODE -eq 0) {
      ...
      Write-Host 'phase5_sync_backend_fastpaths_local_patch=applied'
      return
    }

    & git apply --reverse --check --whitespace=nowarn $patchFile
    if ($LASTEXITCODE -eq 0) {
      throw 'Phase 5 sync backend fastpaths patch appears to be already applied to LocalBackendRoot.'
    }
    throw 'Phase 5 sync backend fastpaths patch cannot be applied cleanly to LocalBackendRoot.'
  } finally {
    Pop-Location
  }
}
...
function Invoke-LocalTests {
  ...
  Push-Location -LiteralPath $BackendRoot
  try {
    Write-Host "Running: go test -count=1 ./modules/message -run 'TestPhase4|TestPhase5|TestSyncSensitiveWords|TestProhibit'"
...
if (-not $Run) {
  ...
  Write-Host 'phase5_sync_backend_fastpaths_sync_backup_dir=<created only when -Run -AllowProductionSync is used>'
  Write-Host 'Reviewed manifest: verified'
  Write-Host 'phase5_sync_backend_fastpaths_reviewed_manifest=verified'
  Write-Host ''
...
if (-not $BuildImage) {
  throw 'Refusing to run production Phase 5 prepare without -BuildImage.'
}

$remoteTempDir = "/tmp/phase5-sync-backend-fastpaths-sync-$([guid]::NewGuid().ToString('N'))"
...
try {
  ...
  $manifestFile = Join-Path ([System.IO.Path]::GetTempPath()) "phase5-sync-backend-fastpaths-manifest-$([guid]::NewGuid().ToString('N')).txt"
  try {
...
if ! diff -u <(printf '%s\n' "`$manifest_text") "`$remote_tmp/.manifest"; then
  echo 'phase5_sync_backend_fastpaths_sync=manifest_mismatch' >&2
  exit 1
fi
...
absent_manifest="`$backup_dir/.phase5_absent_files"
: > "`$absent_manifest"
...
END {
  for (path in before) {
    if (!(path in allowed)) {
      print "phase5_sync_backend_fastpaths_build_context_unreviewed_change=" path > "/dev/stderr"
      exit 1
    }
  }
}' "`$release_paths_file" <(comm -3 "`$remote_tmp/.build-context.before.tsv" "`$remote_tmp/.build-context.after.tsv" | sed 's/^\t//'); then
  echo 'phase5_sync_backend_fastpaths_build_context=unreviewed_change' >&2
  exit 1
fi
echo 'phase5_sync_backend_fastpaths_build_context=verified'

echo 'phase5_sync_backend_fastpaths_sync=applied'
echo "phase5_sync_backend_fastpaths_sync_backup_dir=`$backup_dir"
echo "phase5_sync_backend_fastpaths_absent_files_manifest=`$absent_manifest"
echo 'phase5_sync_backend_fastpaths_reviewed_manifest=verified'
...
if [ "`$build_image" = '1' ]; then
  ...
  cat > "`$build_context_root/Dockerfile.tsdd" <<'PHASE5_DOCKERFILE'
...
ENTRYPOINT ["/home/app"]
PHASE5_DOCKERFILE
  cat > "`$build_context_root/.dockerignore" <<'PHASE5_DOCKERIGNORE'
.git
**/.git
...
**/*.jks
PHASE5_DOCKERIGNORE
  cat > "`$remote_tmp/docker-compose.phase5-build.yaml" <<'PHASE5_COMPOSE'
services:
  tsdd-api:
    build:
...
PHASE5_COMPOSE
  sed -i "s|__BUILD_CONTEXT_ROOT__|`$build_context_root|g" "`$remote_tmp/docker-compose.phase5-build.yaml"
  echo "phase5_sync_backend_fastpaths_build_context_root=`$build_context_root"
  cd "`$remote_production"
  image_timestamp="`$(date -u +%Y%m%dT%H%M%SZ)"
  previous_image_tag="wukongim/tsdd-api:phase5-pre-`$image_timestamp"
  if docker image inspect wukongim/tsdd-api:production-local >/dev/null 2>&1; then
    ...
    echo "phase5_sync_backend_fastpaths_previous_image_tag=`$previous_image_tag"
  else
    echo 'previous production image is missing: wukongim/tsdd-api:production-local' >&2
    exit 1
  fi
  docker compose --env-file .env -f "`$remote_tmp/docker-compose.phase5-build.yaml" build tsdd-api
  echo 'phase5_sync_backend_fastpaths_build=completed'
  docker compose --env-file .env ps tsdd-api
else
  echo 'phase5_sync_backend_fastpaths_build=skipped'
fi
"@
