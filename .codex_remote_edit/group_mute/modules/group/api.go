...
func (g *Group) Route(r *wkhttp.WKHttp) {
	...
	groups := r.Group("/v1/groups", g.ctx.AuthMiddleware(r))
	{
		...
		groups.GET("/:group_no/reminders", g.groupReminderList)                            // 群提醒列表
		groups.POST("/:group_no/reminders", g.groupReminderCreate)                         // 创建群提醒
		groups.PUT("/:group_no/reminders/:id", g.groupReminderUpdate)                      // 编辑群提醒
		groups.DELETE("/:group_no/reminders/:id", g.groupReminderCancel)                   // 取消群提醒
		groups.POST("/:group_no/reminders/:id/done", g.groupReminderDone)                  // 完成群提醒
	}
	authGroups := r.Group("/v1/groups", g.ctx.AuthMiddleware(r))
	{
		authGroups.GET("/:group_no/notice/history", g.groupNoticeHistory)
	}
...
func (g *Group) groupCreate(c *wkhttp.Context) {
	...
	if err := user.RequireVIPEntitlement(creatorUser, user.VIPEntitlementCreateGroup, time.Now()); err != nil {
		c.ResponseError(err)
		return
	}
...
func (g *Group) groupUpdate(c *wkhttp.Context) {
	...
	// 发布群创建事件
	if noticeChanged {
		err = g.db.InsertNoticeHistoryTx(&NoticeHistoryModel{
			GroupNo:      groupNo,
		...
		if err != nil {
			tx.Rollback()
			g.Error("save group notice history failed", zap.Error(err))
			c.ResponseError(errors.New("save group notice history failed"))
			return
		}
	}
	eventID, err := g.ctx.EventBegin(&wkevent.Data{
...
func (g *Group) groupNoticeHistory(c *wkhttp.Context) {
	groupNo := c.Param("group_no")
	loginUID := c.GetLoginUID()
	if groupNo == "" {
		c.ResponseError(errors.New("group no is required"))
		return
	}
	...
	if err != nil {
		g.Error("query group member failed", zap.Error(err))
		c.ResponseError(errors.New("query group member failed"))
		return
	}
	if !exist {
		c.ResponseError(errors.New("only group members can view notice history"))
		return
	}

	items, err := g.db.QueryNoticeHistories(groupNo, limit)
	if err != nil {
		g.Error("query group notice history failed", zap.Error(err))
		c.ResponseError(errors.New("query group notice history failed"))
		return
	}

	resps := make([]groupNoticeHistoryResp, 0, len(items))
	for _, item := range items {
		resps = append(resps, groupNoticeHistoryResp{}.from(item))
	}
	c.Response(resps)
}
...
func (g *Group) memberAdd(c *wkhttp.Context) {
	...
	**/
	...
	operatorUser, err := g.userDB.QueryByUID(operator)
	if err != nil {
		g.Error("查询操作者信息失败！", zap.Error(err), zap.String("uid", operator))
		c.ResponseError(errors.New("查询操作者信息失败"))
		return
	}
	if err := user.RequireVIPEntitlement(operatorUser, user.VIPEntitlementInviteGroupMember, time.Now()); err != nil {
		c.ResponseError(err)
		return
	}
	// 判断是否允许系统账号进入群聊
...
func (g *Group) groupForbidden(c *wkhttp.Context) {
	...
	if forbidden == 1 {
		whitelistUIDs, err = mutedGroupWhitelistUIDs(g.ctx, g.db, groupNo)
		if err != nil {
...
func (g *Group) setIMWhitelistForGroupManager(groupNo string) error {
	...
	return g.resetIMWhitelist(whitelistUIDs, groupNo)
}

func mutedGroupWhitelistUIDs(ctx *config.Context, db *DB, groupNo string) ([]string, error) {
	managerOrCreaterUIDs, err := db.QueryGroupManagerOrCreatorUIDS(groupNo)
	if err != nil {
		return nil, err
	}
	return appendSystemUIDToWhitelist(ctx, managerOrCreaterUIDs), nil
}

func appendSystemUIDToWhitelist(ctx *config.Context, whitelist []string) []string {
	result := make([]string, 0, len(whitelist)+1)
	seen := make(map[string]struct{}, len(whitelist)+1)
	...
	if ctx != nil {
		systemUID := strings.TrimSpace(ctx.GetConfig().Account.SystemUID)
		if systemUID != "" {
			if _, ok := seen[systemUID]; !ok {
				result = append(result, systemUID)
			}
		}
	}
	return result
...
func (g *Group) getGroupInfo(groupNo string) (*Model, error) {
	...
	if group.Status == GroupStatusDisabled {
		return nil, errors.New("VIP已到期，群已禁用")
	}
	creator, err := g.userDB.QueryByUID(group.Creator)
	if err != nil {
		g.Error("查询群创建者VIP状态失败", zap.Error(err), zap.String("group_no", groupNo), zap.String("creator", group.Creator))
		return nil, errors.New("查询群创建者VIP状态失败")
	}
	if user.ShouldExpireVIP(creator, time.Now()) {
		vipRuntime := user.NewVIPRuntime(g.ctx)
		if err := vipRuntime.ExpireMerchantVIP(group.Creator); err != nil {
			g.Error("禁用过期VIP创建的群失败", zap.Error(err), zap.String("group_no", groupNo), zap.String("creator", group.Creator))
			return nil, errors.New("VIP到期处理失败")
		}
		return nil, errors.New("VIP已到期，群已禁用")
	}
	return group, nil
...
// 成员详情model
type groupNoticeHistoryResp struct {
	ID           int64  `json:"id"`
...
func (r groupNoticeHistoryResp) from(model *NoticeHistoryModel) groupNoticeHistoryResp {
	return groupNoticeHistoryResp{
		ID:           model.Id,
		GroupNo:      model.GroupNo,
