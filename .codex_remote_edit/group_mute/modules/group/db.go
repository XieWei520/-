...
func (d *DB) UpdateGroupsCreatedByUIDStatus(uid string, fromStatus, toStatus int) error {
	_, err := d.session.Update("`group`").
		Set("status", toStatus).
		Where("creator=? and status=?", uid, fromStatus).
		Exec()
	return err
}
...
func (d *DB) InsertNoticeHistoryTx(model *NoticeHistoryModel, tx *dbr.Tx) error {
	_, err := tx.InsertInto("group_notice_history").
		Columns(util.AttrToUnderscore(model)...).
		Record(model).
		Exec()
	return err
}

func (d *DB) QueryNoticeHistories(groupNo string, limit uint64) ([]*NoticeHistoryModel, error) {
	if limit == 0 {
		limit = 20
	}
	var models []*NoticeHistoryModel
	_, err := d.session.Select("*").
		From("group_notice_history").
...
type NoticeHistoryModel struct {
	GroupNo      string
	OperatorUID  string
	OperatorName string
	Notice       string
