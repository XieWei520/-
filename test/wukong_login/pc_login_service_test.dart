...
void main() {
  test(
    'requestPCLoginQRCode delegates to LoginBridgeApi.getLoginUuid',
    () async {
  ...
  test(
    'logoutAllSessions and logoutSession delegate to bridge callbacks',
    () async {
      final calls = <String>[];
      final service = PCLoginService(
        requestLoginUuid: () async => const LoginUuidResult(
        ...
        pollLoginStatus: (_) async =>
            const LoginStatusResult(status: 'waitScan'),
        loadDevices: () async => const <LoginBridgeDeviceRecord>[],
