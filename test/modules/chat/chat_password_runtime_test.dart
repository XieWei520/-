...
void main() {
  ...
  test(
    'unlockChat decrements the remaining attempts after a wrong password',
    () async {
  ...
  test(
    'unlockChat resets the remaining attempts after a correct password',
    () async {
      final runtime = ChatPasswordRuntime(
        loadChannel: _loadProtectedChannel,
        clearChannelMessages: (_, _) async {},
  ...
  test(
    'unlockChat clears local messages when attempts are already exhausted',
    () async {
      var clearedChannel = '';
      var clearedType = -1;
      final runtime = ChatPasswordRuntime(
...
Future<WKChannel?> _loadProtectedChannel(
  String channelId,
  int channelType,
) async {
