...
void main() {
  group('Conversation extra parity api', () {
    test(
      'syncExtras parses browse_to keep_message_seq keep_offset_y draft and version',
      () async {
    ...
    test(
      'updateExtra posts Android-aligned body and returns version',
      () async {
        final adapter = _RecordingPlainAdapter(
  ...
  group('Message sync parity api', () {
    test(
      'clearUnread uses Android clearUnread endpoint and unread payload',
      () async {
        final adapter = _RecordingPlainAdapter(
        ...
        await api.clearUnread(channelId: 'group_01', channelType: 2, unread: 7);
    ...
    test(
      'syncMessageExtras posts source device_uuid and extra_version',
      () async {
        final adapter = _RecordingPlainAdapter(
          payload: <String, dynamic>{
            'data': <Map<String, dynamic>>[
    ...
    test(
      'syncCommandMessages posts max_message_seq for offline cmd sync',
      () async {
        final adapter = _RecordingPlainAdapter(
          payload: <String, dynamic>{
